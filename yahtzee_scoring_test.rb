...
class TestYahtzeeScoring < Minitest::Test
  def test_ties
    assert_equal({ categories: [:chance, :four_of_a_kind, :three_of_a_kind], score: 27 }, YahtzeeScoring.best_score([6, 6, 6, 6, 3]))
    assert_equal({ categories: [:chance, :three_of_a_kind], score: 21 }, YahtzeeScoring.best_score([6, 6, 6, 2, 1]))
  end

  def test_yahtzee
    assert_equal({ categories: [:yahtzee], score: 50 }, YahtzeeScoring.best_score([6, 6, 6, 6, 6]))
  ...
  def test_large_straight
    assert_equal({ categories: [:large_straight], score: 40 }, YahtzeeScoring.best_score([2, 3, 4, 5, 6]))
    assert_equal({ categories: [:large_straight], score: 40 }, YahtzeeScoring.best_score([1, 2, 3, 4, 5]))
  ...
  def test_small_straight
    assert_equal({ categories: [:small_straight], score: 30 }, YahtzeeScoring.best_score([1, 2, 3, 4, 3]))
    assert_equal({ categories: [:small_straight], score: 30 }, YahtzeeScoring.best_score([2, 3, 4, 5, 5]))
  ...
  def test_full_house
    assert_equal({ categories: [:full_house], score: 25 }, YahtzeeScoring.best_score([3, 3, 3, 5, 5]))
  ...
  def test_chance
    assert_equal({ categories: [:chance], score: 17 }, YahtzeeScoring.best_score([1, 2, 3, 5, 6]))
    assert_equal({ category: :chance, score: 17 }, YahtzeeScoring.score_chance([1, 2, 3, 5, 6]))
  end

  def test_upper_section
    assert_equal({ categories: [:sixes], score: 12 }, YahtzeeScoring.best_score([6, 6, 1, 1, 2], false))
    assert_equal({ categories: [:fives], score: 10 }, YahtzeeScoring.best_score([5, 4, 4, 5, 1], false))
